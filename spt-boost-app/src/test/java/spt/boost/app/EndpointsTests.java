...
public class EndpointsTests {
	 ...
	 * Property prefix
	 */
	private static final String PROPERTY_PREFIX = "--" + Endpoints.SHUTDOWN_URL_PROPERTY_NAME + "=";
	...
	public void before() throws Exception {
		...
		server.setHandler(new InternalHandler());
		server.start();
	...
	/**
	 * {@link Endpoints#shutdown(String...)}
	 */
	@Test
	public void shutdown() {
		
		// No arguments
		assertThat(Endpoints.shutdown("")).isEqualTo(false);
		
		// Invalid URL
		try {
			
			Endpoints.shutdown(PROPERTY_PREFIX + "-----");
		...
		// OK
		assertThat(Endpoints.shutdown(PROPERTY_PREFIX + this.url + "/200")).isEqualTo(true);
		
		// Not OK
		try {
			
			Endpoints.shutdown(PROPERTY_PREFIX + this.url);
		...
		// Connection error
		assertThat(Endpoints.shutdown(PROPERTY_PREFIX + "http://foo.bar.baz.qux/")).isEqualTo(false);
	}
	
	/**
	 * Internal handler
	 */
	protected static class InternalHandler extends AbstractHandler {
